...
public class BinaryAPIControllerTest {
	...
	public void multiply() throws Exception {
		this.mvc.perform(get("/multiply").param("operand1","111").param("operand2","10"))
			.andExpect(status().isOk())
	...
	public void multiplyJson() throws Exception {
		this.mvc.perform(get("/multiply_json").param("operand1","111").param("operand2","10"))
			.andExpect(status().isOk())
			.andExpect(MockMvcResultMatchers.jsonPath("$.operand1").value("111"))
	...
	public void multiplyWithZero() throws Exception {
		this.mvc.perform(get("/multiply").param("operand1","0").param("operand2","101010"))
			.andExpect(status().isOk())
			.andExpect(content().string("0"));
	}

	@Test
	public void andWithInvalidOperand() throws Exception {
		this.mvc.perform(get("/and").param("operand1","10x1").param("operand2","1010"))
			.andExpect(status().isOk())
			.andExpect(content().string("0"));
	}

	@Test
	public void orWithLeadingZeros() throws Exception {
		this.mvc.perform(get("/or").param("operand1","0011").param("operand2","0100"))
			.andExpect(status().isOk())
